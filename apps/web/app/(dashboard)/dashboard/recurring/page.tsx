...
export default async function RecurringInvoicesPage({
  ...
  const recurringInvoices = await prisma.recurringInvoice.findMany({
    where: { 
      organizationId,
      customer: q ? { name: { contains: q, mode: 'insensitive' } } : undefined
    },
    ...
    <div className="p-8 max-w-6xl mx-auto w-full space-y-8 animate-in fade-in duration-700">
      ...
      <form method="GET" className="flex flex-col md:flex-row gap-4 items-center justify-between">
        ...
        <div className="flex items-center gap-2 w-full md:w-auto">
          ...
          <Link href="/dashboard/recurring" className="p-2.5 bg-secondary border border-border rounded-xl text-xs font-bold hover:bg-border transition-all">
            Reset
          </Link>
        </div>
      </form>
                ...
                <tr>
                  <td colSpan={6} className="px-8 py-24 text-center">
                    <div className="flex flex-col items-center gap-3">
                      ...
                      <p className="text-sm font-bold text-foreground">No schedules found</p>
                      <p className="text-xs text-muted-foreground max-w-[250px]">Try adjusting your search or create a new automation schedule.</p>
                    </div>
                  </td>
                </tr>
              ) : (
                recurringInvoices.map((recurring) => (
                  <tr key={recurring.id} className="hover:bg-secondary/20 transition-all group">
                    ...
                    <td className="px-8 py-6 text-right">
                      <Link href={`/dashboard/recurring/${recurring.id}`} className="p-2 hover:bg-secondary rounded-xl transition-colors text-muted-foreground hover:text-foreground inline-block">
