...
export default function RecurringForm({ customers, orgStateCode, defaultDueDays }: RecurringFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        setErr(data.error || "Failed to create subscription");
        setLoading(false);
        return;
      }

      setSuccess(true);
      setTimeout(() => router.push("/dashboard/recurring"), 1500);
    } catch (e: any) {
    ...
    <form onSubmit={handleSubmit} className="space-y-8">
      ...
      <div className="bg-card border border-border rounded-2xl p-6 space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="block text-xs font-bold text-muted-foreground mb-2 uppercase tracking-wider">Title (Optional)</label>
            <input
              type="text"
              value={title}
            ...
            <div className="flex gap-2">
              {["WEEKLY", "MONTHLY", "QUARTERLY", "YEARLY"].map((freq) => (
                <button
                  key={freq}
                  type="button"
                  onClick={() => setInterval(freq as any)}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="block text-xs font-bold text-muted-foreground mb-2 uppercase tracking-wider">First Invoice Date</label>
            <input
              type="date"
              value={nextIssueDate}
          ...
          <div>
            <label className="block text-xs font-bold text-muted-foreground mb-2 uppercase tracking-wider">End Date (Optional)</label>
            <input
              type="date"
              value={endDate}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <input
              type="number"
              value={dueDays}
              onChange={(e) => setDueDays(Math.max(1, parseInt(e.target.value) || 0))}
              min="1"
              className="w-full bg-background border border-border rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary"
            />
          </div>
          ...
          {items.map((item, idx) => (
            <div key={item.id} className="flex gap-3 items-end pb-3 border-b border-border last:border-0">
              <div className="flex-1">
                <label className="block text-[10px] font-bold text-muted-foreground mb-1 uppercase tracking-widest">Description</label>
                <input
                  type="text"
              ...
              <div className="w-20">
                <label className="block text-[10px] font-bold text-muted-foreground mb-1 uppercase tracking-widest">HSN</label>
                <input
                  type="text"
                  value={item.hsnCode}
              ...
              <div className="w-16">
                <label className="block text-[10px] font-bold text-muted-foreground mb-1 uppercase tracking-widest">Qty</label>
                <input
                  type="number"
                  value={item.quantity}
              ...
              <div className="w-24">
                <label className="block text-[10px] font-bold text-muted-foreground mb-1 uppercase tracking-widest">Price</label>
                <input
                  type="number"
              ...
              <div className="w-16">
                <label className="block text-[10px] font-bold text-muted-foreground mb-1 uppercase tracking-widest">Tax %</label>
                <input
                  type="number"
                  value={item.taxRate}
      ...
      <div className="bg-primary/5 border border-primary/20 rounded-2xl p-6">
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          <div>
            ...
            <p className="text-lg font-black text-foreground">₹{totals.subTotal.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</p>
          </div>
          {!isInterState && (
            <>
              <div>
                <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-1">CGST</p>
                <p className="text-lg font-black text-foreground">₹{totals.cgstTotal.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</p>
              </div>
              <div>
                <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-1">SGST</p>
                <p className="text-lg font-black text-foreground">₹{totals.sgstTotal.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</p>
              </div>
            </>
          )}
          {isInterState && (
            <div>
              ...
              <p className="text-lg font-black text-foreground">₹{totals.igstTotal.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</p>
            </div>
          )}
          <div className="col-span-2 md:col-span-1 text-right">
            <p className="text-[10px] font-bold text-primary uppercase tracking-widest mb-1">Total</p>
            <p className="text-2xl font-black text-primary">₹{totals.grandTotal.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex gap-3">
        ...
        >
          ...
          {success ? "Created!" : "Create Subscription"}
        </button>
      </div>
    </form>
  );
